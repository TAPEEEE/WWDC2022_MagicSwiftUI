...
struct StackView: View {
    ...
    var CodeText: [String] = ["VStack", "HStack", "ZStack"]

    var body: some View {
        VStack {
            PreviewStackView(ShowStackPreview: TextBox)
            ...
            VStack {
                HStack {
                    Group {
                        Text("var ").foregroundColor(Color("TextColor1")) +
                        Text("body: ").foregroundColor(Color("TextColor2")) +
                        Text("some  ").foregroundColor(Color("TextColor1")) +
                        Text("View").foregroundColor(Color("TextColor2"))
                        Text("{")
                    }
                }
                ...
                HStack {
                    RoundedRectangle(cornerRadius: 10).strokeBorder(StrokeBox,lineWidth: 2)
                        ...
                        .onDrop(of: ["public.text"], delegate: StackDropDelegate(color: $backgroundColor, StrokeColor: $StrokeBox, Text: $TextBox))
        ...
        if let item = info.itemProviders(for: ["public.text"]).first {
            item.loadItem(forTypeIdentifier: "public.text", options: nil) { (text, err) in
                if let data = text as? Data {
                    ...
                    } else if inputStr == "HStack" {
                        self.color = Color(UIColor.systemGray5)
                        self.Text = "HStack"
                        self.StrokeColor = .green
                    } else if inputStr == "ZStack" {
                        self.Text = "ZStack"
                        self.color = Color(UIColor.systemGray5)
                        self.StrokeColor = .green
                    }
